...
DO $$ BEGIN
    CREATE TYPE public.statoofferta AS ENUM ('In Sospeso', 'Accettata', 'Rifiutata');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
    CREATE TYPE public.statovisita AS ENUM ('In Sospeso', 'Confermata', 'Rifiutata');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
CREATE TABLE IF NOT EXISTS public.agenzia (
    idagenzia integer NOT NULL,
    CONSTRAINT agenzia_pkey PRIMARY KEY (idagenzia)
);

CREATE SEQUENCE IF NOT EXISTS public.agenzia_idagenzia_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
CREATE TABLE IF NOT EXISTS public.utenti (
    ...
    CONSTRAINT utenti_pkey PRIMARY KEY (uid),
    CONSTRAINT utenti_email_unique UNIQUE (email)
);

CREATE TABLE IF NOT EXISTS public.utenteagenzia (
    uid       character varying(255) NOT NULL,
    idagenzia integer                NOT NULL,
    CONSTRAINT utenteagenzia_pkey PRIMARY KEY (uid)
);
...
CREATE TABLE IF NOT EXISTS public.immobile (
    ...
    CONSTRAINT immobile_pkey PRIMARY KEY (idimmobile)
);

CREATE SEQUENCE IF NOT EXISTS public.immobile_idimmobile_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
CREATE TABLE IF NOT EXISTS public.offerta (
    idofferta  integer                NOT NULL,
    importo    double precision       NOT NULL,
    stato      public.statoofferta    NOT NULL,
    idcliente  character varying(255) NOT NULL,
    idimmobile integer                NOT NULL,
    CONSTRAINT offerta_pkey PRIMARY KEY (idofferta)
);

CREATE SEQUENCE IF NOT EXISTS public.offerta_idofferta_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
CREATE SEQUENCE IF NOT EXISTS public.visita_idvisita_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
DO $$ BEGIN
    ALTER TABLE ONLY public.utenteagenzia
        ADD CONSTRAINT fk_utenteagenzia_utenti FOREIGN KEY (uid) REFERENCES public.utenti(uid) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
    ALTER TABLE ONLY public.offerta
        ADD CONSTRAINT fk_offerta_cliente FOREIGN KEY (idcliente) REFERENCES public.utenti(uid);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
    ALTER TABLE ONLY public.visita
        ADD CONSTRAINT fk_visita_cliente FOREIGN KEY (idcliente) REFERENCES public.utenti(uid);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_immobile_prezzo_asc      ON public.immobile USING btree (prezzo);
CREATE INDEX IF NOT EXISTS idx_immobile_prezzo_desc     ON public.immobile USING btree (prezzo DESC);
CREATE INDEX IF NOT EXISTS idx_immobile_dimensione_asc  ON public.immobile USING btree (dimensione);
CREATE INDEX IF NOT EXISTS idx_immobile_dimensione_desc ON public.immobile USING btree (dimensione DESC);
CREATE INDEX IF NOT EXISTS idx_immobile_nbagni_asc      ON public.immobile USING btree (nbagni);
CREATE INDEX IF NOT EXISTS idx_immobile_nbagni_desc     ON public.immobile USING btree (nbagni DESC);
CREATE INDEX IF NOT EXISTS idx_immobile_nstanze_asc     ON public.immobile USING btree (nstanze);
CREATE INDEX IF NOT EXISTS idx_immobile_nstanze_desc    ON public.immobile USING btree (nstanze DESC);

INSERT INTO public.agenzia (idagenzia) VALUES (1), (2) ON CONFLICT DO NOTHING;
