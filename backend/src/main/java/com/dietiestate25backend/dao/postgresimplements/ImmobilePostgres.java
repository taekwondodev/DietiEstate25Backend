...
public class ImmobilePostgres implements ImmobileDao {
    ...
    public Immobile getImmobileById(int id) {
        String sql = "SELECT * FROM immobile WHERE idImmobile = ?";
        return  jdbcTemplate.queryForObject(sql, new ImmobileRowMapper(), id);
    }
    ...
    private static class ImmobileRowMapper implements RowMapper<Immobile> {
        ...
        public Immobile mapRow(ResultSet rs, int rowNum) throws SQLException {
            return Immobile.builder()
                    .idImmobile(rs.getInt(IDIMMOBILE))
                    .urlFoto(rs.getString(URLFOTO))
                    .descrizione(rs.getString(DESCRIZIONE))
                    .prezzo(rs.getDouble(PREZZO))
                    .dimensione(rs.getInt(DIMENSIONE))
                    .nBagni(rs.getInt(N_BAGNI))
                    .nStanze(rs.getInt(N_STANZE))
                    ...
                    .hasAscensore(rs.getBoolean(HAS_ASCENSORE))
                    .hasBalcone(rs.getBoolean(HAS_BALCONE))
                    .idResponsabile(rs.getString(ID_AGENTE))
                    .build();
        }
    }
}
